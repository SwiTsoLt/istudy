...
export class GatewayService {
  ...
  public removeRoom(socket: Socket): interfaces.IMessage {
    try {
      ...
      if (clientSocket) {
        ...
        clientSocket.send({ type: 'success', msg: messageEnum.leaveRoomSuccess });
      }
  ...
  public leaveRoom(socket: Socket): interfaces.IMessage {
    try {
      ...
      if (ownerSocket) {
        ...
        ownerSocket.send({ type: 'success', msg: messageEnum.leaveRoomSuccess });
      }
  ...
  public rtcData(socket: Socket, rtcData: RtcDataDto): interfaces.IMessage | null {
    try {
      const client = this.clients[socket.id];
